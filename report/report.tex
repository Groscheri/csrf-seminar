...
?>
  ...
  To make it even worse, the server didn't check if the call came from the same webpage~\cite{gmail_contact_list_csrf}. A simple popup window could 
  have triggered this attack.
  
  % Alternative ref: 
  % [4] http://ajaxian.com/archives/gmail-csrf-security-flaw ==> not cited in the text
  % Accessed: 2015-11-24
  % TODO add \cite{}s in the text and remove \nocite{}s below
  ...
  \begin{figure}[h!t]
  \begin{minted}[linenos]{html} % JS = JavaScript
  ...
  The code in Figure~\ref{figure:callback} does a very basic operation. The attacker could as well send the data to another program 
  ...
  on the subject and this seminar didn't intend to rewrite what was already done. The aim was to deal with a real subject 
  ...
  Impacts of web security breaches are more and more important. Attackers don't want to destroy servers or deface 
  websites anymore. They are trying to invisibly take control of servers in order to gather information. ``Data is gold'' and 
  attackers do their best in order to get access to it, the reasons are plethora: money, intelligence, challenge, etc. Prevent 
  systems from these attacks is one of the most important security matter since the beginning of Internet but even more with 
  the emergence of Internet of Things (IoT), Bring Your Own Device (BYOD) concept and growing exposure of IT systems.
